...
- `AcctControlMaster`: prototype is a flat 28-row table. **2026-05-17: the real routing matrix was delivered** — `VRS_DATA_ROUND_4.xlsx` (on share), **2,842 rows**, 203 `rebate_type`s × routing key → ~14 ledger lines across RSL/GL/AP × Accrual/Reclass/Prepaid/Received with real account/offset/cost-center values + reverse flags (`docs/19` K3, RESOLVED). No longer "make it look real" — **load it real** in P0.6; also now the authoritative `rebate_type`/`acct_type` domain. (Earlier "3-table `_DETAIL`/`_FUNCTIONS` / fixed 10-transaction" framing from `docs/17` image067 is superseded by the actual extract.)
